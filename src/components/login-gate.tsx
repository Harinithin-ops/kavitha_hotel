...
export function LoginGateProvider({ children }: { children: React.ReactNode }) {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    // Persist to Supabase (non-blocking — don't show error to user)
    saveUser({ name: newUser.name, email: newUser.email, phone: newUser.phone, source: "login-modal" }).catch(() => {})

    setOpen(false)
    toast({ title: `Welcome, ${newUser.name}! 👋`, description: "You can now continue booking." })
  ...
  const setAdminSession = useCallback(() => {
    const adminUser: LoginUser = { name: "Admin", email: "harinithins28@gmail.com", isAdmin: true }
    try {
      localStorage.setItem("hk_user", JSON.stringify(adminUser))
      setUser(adminUser)
    } catch {}
    toast({ title: "Admin Login Successful", description: "You now have admin privileges." })
  }, [toast])
  ...
  const value = useMemo(() => ({ user, requireLogin, setAdminSession, logout }), [user, requireLogin, setAdminSession, logout])
